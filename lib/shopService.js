...
function normalizeImageUrls(value, fallback = '') {
  ...
  const cleanUrls = urls.map((item) => String(item).trim()).filter(Boolean).slice(0, 3);
  if (!cleanUrls.length && fallback) return [fallback];
  return cleanUrls;
}
...
function mapProduct(item) {
  ...
  return {
    ...
    discount_price: parsePrice(item.discount_price),
...
export function formatProductPrice(item) {
  const value = item?.discount_price ?? item?.price_value;
...
export async function fetchPublicShopProducts(businessId = null) {
  let query = supabase
    .from('shop_products')
    .select('*')
    .eq('is_active', true)
